...
| | 离散动作 | 连续动作 |
|---|---|---|
| **定义** | 从有限选项中选一个 | 输出精确的连续数值 |
| **c4nav-core 示例** | 5 选 1：左转45°/左转22.5°/直行/右转22.5°/右转45° | 输出 2 个浮点数：舵角 [-45°, +45°]、油门 [0, 1.0] |
| **类比** | 方向盘只能打到 5 个固定位置 | 方向盘可以打到任意角度 |
| **精度** | 粗（只有 5 档） | 细（无限精度） |
| **兼容算法** | DQN 系列、PPO、A2C | PPO、A2C、SAC、TD3、DDPG |
| **训练难度** | 低（搜索空间小） | 高（搜索空间无限大） |
...
- C4 竞赛场景中，5 个离散动作已经够用：避障本质上就是"左转/右转/直行"的决策
- 训练快、收敛稳定、容易调试
- 83% 成功率说明离散动作空间不是瓶颈

**实船部署阶段 → 连续更好**

- 真实舵机接收的是连续角度信号，离散动作需要额外映射
- 连续动作可以输出"右转 17.3°、油门 62%"这样精细的控制
- 高速航行时，5 档转向太粗糙，可能导致蛇行（左右摆动）
- 风浪中需要持续微调，不是"打到位就不动"
...
| 场景 | 离散 (5 档) | 连续 |
|------|-----------|------|
| 开阔水域直行 | 足够（选"直行"即可） | 无明显优势 |
| 窄通道穿越 | 可能不够精细（最小转角 22.5°） | 可以微调到 5°、10° |
| 高速急转 | 只有 45° 一档，要么不转要么大转 | 可以输出 30° 等中间值 |
| 风浪补偿 | 反复在"左转/直行"之间切换，抖动 | 持续输出 2°~3° 微修正 |

!!! tip "扩展路径"
    如果将 c4nav-core 的动作空间从离散 (5) 改为连续 `Box(2,)`（舵角 + 油门），则可以引入 SAC、TD3 等连续动作算法。但 DQN 系列将不再适用。代码架构已支持这种扩展（算法-环境解耦，改 YAML 配置即可）。
