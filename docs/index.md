...
```
 ...
 +----------------+  +----------------+  +----------------+  +----------------+
 |  学习 ID3QN    |  |  触发 GitHub   |  |  解读图表      |  |  数据映射到    |
 |  设计与改进     |--->  Actions 训练  |--->  CSV 与模型   |--->  图、表、论文  |
 +----------------+  +----------------+  +----------------+  +----------------+
         |                  |                  |                  |
   DQN 系列详解        云端训练指南        训练结果解读        论文写作指南
```

| 步骤 | 页面 | 你将学到 |
|:---:|---|---|
| **1** | [**DQN 系列详解**](algorithms/dqn-family.md) | DQN -> DDQN -> D3QN -> ID3QN 递进设计与代码对照 |
| **2** | [**云端训练**](guides/cloud-training.md) | 如何通过 GitHub Actions 触发云端训练 |
...
快速开始、GPU 配置、云端训练、训练结果解读、论文写作、YAML 参数配置参考。
