...
沿以下 4 步体验**完整的科研流程** -- 从算法原理到发表论文：

```
 1. 算法原理         2. 训练模型          3. 理解结果         4. 撰写论文

 +----------------+  +----------------+  +----------------+  +----------------+
 |  学习 ID3QN    |  |  本地/GitHub/  |  |  解读图表      |  |  数据映射到    |
 |  设计与改进     |--->  Colab 训练   |--->  CSV 与模型   |--->  图、表、论文  |
 +----------------+  +----------------+  +----------------+  +----------------+
         |                  |                  |                  |
   DQN 系列详解          训练方式          训练结果解读        论文写作指南
```

| 步骤 | 页面 | 你将学到 |
|:---:|---|---|
| **1** | [**DQN 系列详解**](algorithms/dqn-family.md) | DQN -> DDQN -> D3QN -> ID3QN 递进设计与代码对照 |
| **2** | [**训练方式**](guides/training-methods.md) | 本地、GitHub Actions、Google Colab 三种训练方式 |
| **3** | [**训练结果解读**](guides/understanding-results.md) | 如何解读训练曲线、柱状图、轨迹图与 CSV |
| **4** | [**论文写作指南**](guides/paper-writing.md) | 训练数据如何变成图、表和论文章节 |
...
快速开始、GPU 配置、训练方式、训练结果解读、论文写作、YAML 参数配置参考。
